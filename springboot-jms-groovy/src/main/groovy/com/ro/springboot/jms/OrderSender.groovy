...
//anonymous inner class converted to groovy closure 
...
//			}});
        ...
        jmsTemplate.convertAndSend(ORDER_QUEUE, order, { it ->

            log.info "setting standard JMS headers before sending" 
		
            it.setJMSCorrelationID UUID.randomUUID().toString() 
            it.setJMSExpiration 1000 
            it.setJMSMessageID "message-id" 
            it.setJMSDestination new ActiveMQQueue(ORDER_QUEUE) 
            it.setJMSReplyTo new ActiveMQQueue(ORDER_QUEUE) 
            it.setJMSDeliveryMode DeliveryMode.NON_PERSISTENT 
            it.setJMSPriority Message.DEFAULT_PRIORITY 
            it.setJMSTimestamp System.nanoTime() 
            it.setJMSType "type" 

            log.info "setting custom JMS headers before sending" 
            it.setStringProperty "jms-custom-header", "this is a custom jms property"
            it.setBooleanProperty "jms-custom-property", true 
            it.setDoubleProperty "jms-custom-property-price", 5.0
